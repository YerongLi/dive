...
def register_receivables(input_string):
    ...
    for x in l:
        ...
        m[(merchant_id,card_type,payout_date)]+= int(amount)
    ansstr = [f'{merchant_id},{card_type},{payout_date},{m[(merchant_id,card_type,payout_date)]}'for merchant_id,card_type,payout_date in m]
    ansstr = '\n'.join(ansstr)
...
def update_receivables(registered_csv, contracts_csv):
    ...
    for x in l:
        ...
        m[(contract_id,card_type,payout_date)]+= int(amount)
        m[(merchant_id,card_type,payout_date)]-= int(amount)
    ansstr = [f'{i},{card_type},{payout_date},{m[(i,card_type,payout_date)]}'for i,card_type,payout_date in m if m[(i,card_type,payout_date)] > 0]
    ansstr.sort()
    ansstr = '\n'.join(ansstr)
